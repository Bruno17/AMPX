...
class AMP_Img_Sanitizer extends AMP_Base_Sanitizer {
	...
	private static $anim_extension = '.gifXX';
	...
	public function sanitize() {
		...
		for ( $i = $num_nodes - 1; $i >= 0; $i-- ) {
            ...
            $new_attributes['src'] = AMP_Image_Dimension_Extractor::normalize_url($new_attributes['src']);
             
			// Try to extract dimensions for the image, if not set.
			if ( ! isset( $new_attributes['width'] ) || ! isset( $new_attributes['height'] ) ) {
			    if (strstr($new_attributes['src'],'[[')){
			        //if we have unprocessed tags in the src, try it later again
			        continue; 
			    }
